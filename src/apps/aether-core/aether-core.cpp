...
#include "aether/ipc/CoreIpcServer.h"
...
int main(int argc, char **argv)
{
  ...
  if (!sharedMemory.create(1) && parser.singleInstanceOnly()) {
    LOG_WARN("an instance of aether core is already running");
  ...
  const auto ipcServer = new aether::core::ipc::CoreIpcServer(&app); // NOSONAR - Qt managed
  QObject::connect(
      ipcServer, &aether::core::ipc::IpcServer::stopProcessRequested, coreApp, &App::quit, Qt::DirectConnection
  );
  ipcServer->listen();

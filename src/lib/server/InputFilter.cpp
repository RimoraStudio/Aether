...
{
  return aether::string::sprintf("keystroke(%s)", aether::KeyMap::formatKey(m_key, m_mask).c_str());
}
...
{
  std::string key = aether::KeyMap::formatKey(kKeyNone, m_mask);
  if (!key.empty()) {
    key += "+";
  }
  return aether::string::sprintf("mousebutton(%s%d)", key.c_str(), m_button);
}
...
{
  return aether::string::sprintf("connect(%s)", m_screen.c_str());
}
...
std::string InputFilter::LockCursorToScreenAction::format() const
{
  ...
  return aether::string::sprintf("lockCursorToScreen(%s)", s_mode[m_mode]);
}
...
std::string InputFilter::RestartServer::format() const
{
  ...
  return aether::string::sprintf("restartServer(%s)", s_mode[m_mode]);
}
...
{
  return aether::string::sprintf("switchToScreen(%s)", m_screen.c_str());
}
...
std::string InputFilter::SwitchInDirectionAction::format() const
{
  ...
  return aether::string::sprintf("switchInDirection(%s)", s_names[static_cast<int>(m_direction)]);
}
...
std::string InputFilter::KeyboardBroadcastAction::format() const
{
  ...
  if (m_screens.empty() || m_screens[0] == '*') {
    return aether::string::sprintf("%s(%s)", s_name, s_mode[m_mode]);
  } else {
    return aether::string::sprintf(
...
std::string InputFilter::KeystrokeAction::format() const
{
  ...
  if (m_keyInfo->m_screens[0] == '\0') {
    return aether::string::sprintf(
        "%s(%s)", type, aether::KeyMap::formatKey(m_keyInfo->m_key, m_keyInfo->m_mask).c_str()
    );
    ...
    return aether::string::sprintf(
        "%s(%s,%s)", type, aether::KeyMap::formatKey(m_keyInfo->m_key, m_keyInfo->m_mask).c_str(),
...
std::string InputFilter::MouseButtonAction::format() const
{
  ...
  std::string key = aether::KeyMap::formatKey(kKeyNone, m_buttonInfo.m_mask);
  return aether::string::sprintf("%s(%s%s%d)", type, key.c_str(), key.empty() ? "" : "+", m_buttonInfo.m_button);
}

...
#include "aether/KeyTypes.h"
#include "aether/OptionTypes.h"
...
void Config::setAetherAddress(const NetworkAddress &addr)
{
  m_aetherAddress = addr;
}
...
const NetworkAddress &Config::getAetherAddress() const
{
  return m_aetherAddress;
}
...
{
  if (m_aetherAddress != x.m_aetherAddress) {
    return false;
  }
...
std::string Config::formatInterval(const Interval &x)
{
  ...
  return aether::string::sprintf("(%d,%d)", (int)(x.first * 100.0f + 0.5f), (int)(x.second * 100.0f + 0.5f));
}
...
std::string Config::getOptionValue(OptionID id, OptionValue value)
{
  ...
  if (id == kOptionHeartbeat || id == kOptionScreenSwitchCornerSize || id == kOptionScreenSwitchDelay ||
    ...
    return aether::string::sprintf("%d", value);
  }
  if (id == kOptionScreenSwitchCorners) {
...
std::ostream &operator<<(std::ostream &s, const Config &config)
{
  ...
  if (config.m_aetherAddress.isValid()) {
    s << "\taddress = " << config.m_aetherAddress.getHostname().c_str() << std::endl;
  }
    ...
    if (!line.empty()) {
      ...
      for (i = 0; i < line.length(); ++i) {
        if (!isgraph(line[i]) && line[i] != ' ' && line[i] != '\t') {
          throw ServerConfigReadException(*this, "invalid character %{1}", aether::string::sprintf("%#2x", line[i]));
        }
      }
...
ConfigReadContext::parseKeystroke(const std::string &keystroke, const std::set<std::string> &screens) const
{
  ...
  if (!aether::KeyMap::parseModifiers(s, mask)) {
    throw ServerConfigReadException(*this, "unable to parse key modifiers");
  }

  KeyID key;
  if (!aether::KeyMap::parseKey(s, key)) {
...
ServerConfigReadException::ServerConfigReadException(const ConfigReadContext &context, const std::string &error)
    : m_error(aether::string::sprintf("line %d: %s", context.getLineNumber(), error.c_str()))
{
  // do nothing
}
...
ServerConfigReadException::ServerConfigReadException(
    ...
    : m_error(
          aether::string::sprintf("line %d: ", context.getLineNumber()) +
          aether::string::format(errorFmt, arg.c_str())
      )
{
  // do nothing
}

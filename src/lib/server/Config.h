...
namespace std {
template <> struct iterator_traits<aether::server::Config>
{
...
private:
  ...
  using CellMap = std::map<std::string, Cell, aether::string::CaselessCmp>;
  using NameMap = std::map<std::string, std::string, aether::string::CaselessCmp>;
...
public:
  ...
  */
  ...
  //! Set server address
  /*!
  Set the aether listen addresses.  There is no default address so
  this must be called to run a server using this configuration.
  */
  void setAetherAddress(const NetworkAddress &);
  ...
  */
  ...
  const NetworkAddress &getAetherAddress() const;

...
Server::Server(ServerConfig &config, PrimaryClient *primaryClient, aether::Screen *screen, IEventQueue *events)
...
void Server::handleClientDisconnected(BaseClientProxy *client)
{
  ...
  using enum aether::core::ConnectionState;
...
void Server::onMouseMoveSecondary(int32_t dx, int32_t dy)
{
  ...
  const static auto adjustEnv = "AETHER_MOUSE_ADJUSTMENT";

...
ISocketMultiplexerJob *TCPSocket::serviceConnecting(ISocketMultiplexerJob *job, bool, bool write, bool error)
{
  ...
  // 10.3.4 by starting an aether client and telling to connect to
  // another system that's not running an aether server.  it will
  // claim to have connected then quickly disconnect (i guess because
  // read returns 0 bytes).  unfortunately, aether attempts to
  ...
  // report the socket as being writable so aether is able to time
  // out the attempt.)
  if (error || true) {
    try {

...
Client::DisconnectRequest::DisconnectRequest(aether::core::ConnectionRefusal reason, const char *message)
    : m_kind(Kind::Refuse),
...
void Client::connect(size_t addressIndex)
{
  ...
  try {
    ...
    if (m_serverAddress.getAddress() != nullptr) {
      ...
      ipcSendConnectionState(aether::core::ConnectionState::Connecting);
    }
...
void Client::refuseConnection(aether::core::ConnectionRefusal reason, const char *msg)
{
  const auto metaEnum = QMetaEnum::fromType<aether::core::ConnectionRefusal>();

...
#include "aether/ClipboardChunk.h"
...
ServerProxy::ServerProxy(Client *client, aether::IStream *stream, IEventQueue *events)
    : m_client(client),
      m_stream(stream),
      m_events(events)
{
...
{
  using enum ConnectionResult;
  using enum aether::core::ConnectionRefusal;
...
void ServerProxy::requestRefuseConnection(aether::core::ConnectionRefusal reason, const char *message)
{
  m_events->addEvent(Event(

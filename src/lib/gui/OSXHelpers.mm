...
bool isOSXDevelopmentBuild()
{
  ...
  return (bundleURL.find("Applications/Aether.app") == std::string::npos);
}
...
static NSApplicationTerminateReply aether_applicationShouldTerminate(id self, SEL _cmd, NSApplication *sender)
{
...
void installQuitHandler(std::function<bool()> shouldQuit)
{
  ...
  if (method) {
    ...
    method_setImplementation(method, (IMP)aether_applicationShouldTerminate);
  } else {
    class_addMethod(cls, selector, (IMP)aether_applicationShouldTerminate, "l@:@");
  }

...
bool ServerConfigDialog::browseConfigFile()
{
  ...
  const auto aetherConfigFilter = tr("%1 Configurations (*.conf);;All files (*.*)");

  QString fileName =
      QFileDialog::getOpenFileName(this, tr("Browse for a config file"), "", aetherConfigFilter.arg(kAppName));

 ...
 * SPDX-FileCopyrightText: (C) 2024 - 2026 Rimora Studio
...
PortalInputCapture::mapPortalActivationToScreenPosition(guint barrierId, double rawX, double rawY) const
{
  ...
  // The portal reports per-output zones, while Aether models the whole computer as one screen.
  // Use the activated barrier to preserve the intended switch direction in Aether's aggregate coordinates.
  using enum BarrierSide;
  switch (it->side) {
  case Left:
    x = screenX;

...
std::string XWindowsUtil::atomToString(Display *display, Atom atom)
{
  ...
  if (error) {
    return aether::string::sprintf("<UNKNOWN> (%d)", (int)atom);
  } else {
    std::string msg = aether::string::sprintf("%s (%d)", name, (int)atom);
    XFree(name);
    return msg;
  }
}
...
std::string XWindowsUtil::atomsToString(Display *display, const Atom *atom, uint32_t num)
{
  ...
  if (error) {
    for (uint32_t i = 0; i < num; ++i) {
      msg += aether::string::sprintf("<UNKNOWN> (%d), ", (int)atom[i]);
    }
  } else {
    for (uint32_t i = 0; i < num; ++i) {
      msg += aether::string::sprintf("%s (%d), ", names[i], (int)atom[i]);
      XFree(names[i]);
    }
  }
  delete[] names;

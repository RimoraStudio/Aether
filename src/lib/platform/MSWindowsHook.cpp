...
static bool keyboardHookHandler(WPARAM wParam, LPARAM lParam)
{
  ...
  if (wParam == AETHER_HOOK_FAKE_INPUT_VIRTUAL_KEY && ((lParam >> 16) & 0xffu) == AETHER_HOOK_FAKE_INPUT_SCANCODE) {
    // update flag
    g_fakeServerInput = ((lParam & 0x80000000u) == 0);
    PostThreadMessage(g_threadID, AETHER_MSG_DEBUG, 0xff000000u | wParam, lParam);
  ...
  if (g_fakeServerInput) {
    PostThreadMessage(g_threadID, AETHER_MSG_DEBUG, 0xfe000000u | wParam, lParam);
    return false;
  }
  ...
  PostThreadMessage(g_threadID, AETHER_MSG_DEBUG, wParam, lParam);
  ...
  if ((g_deadVirtKey == wParam || g_deadRelease == wParam) && (lParam & 0x80000000u) != 0) {
    g_deadRelease = 0;
    PostThreadMessage(g_threadID, AETHER_MSG_DEBUG, wParam | 0x04000000, lParam);
    return false;
  }
  ...
  // we note that's the case in the message sent back to aether
  // because there's no simple way to deduce it after the fact.
  // we have to put the dead key back first, if there was one.
  bool noAltGr = false;
  if (n == 0 && (control & 0x80) != 0 && (menu & 0x80) != 0) {
    noAltGr = true;
    PostThreadMessage(g_threadID, AETHER_MSG_DEBUG, wParam | 0x05000000, lParam);
    setDeadKey(wc, 2, flags);
  ...
  PostThreadMessage(
      g_threadID, AETHER_MSG_DEBUG, (wc[0] & 0xffff) | ((wParam & 0xff) << 16) | ((n & 0xf) << 24) | 0x60000000,
      lParam
  );
  ...
  case 2: {
    ...
    PostThreadMessage(g_threadID, AETHER_MSG_KEY, deadCharAndVirtKey, g_deadLParam & 0x7fffffffu);
    PostThreadMessage(g_threadID, AETHER_MSG_KEY, deadCharAndVirtKey, g_deadLParam | 0x80000000u);
  ...
  // forwarding.
  if (charAndVirtKey != 0) {
    PostThreadMessage(g_threadID, AETHER_MSG_DEBUG, charAndVirtKey | 0x07000000, lParam);
    PostThreadMessage(g_threadID, AETHER_MSG_KEY, charAndVirtKey, lParam);
  }
...
static bool mouseHookHandler(WPARAM wParam, int32_t x, int32_t y, int32_t data)
{
  ...
  case WM_NCXBUTTONUP:
    ...
    PostThreadMessage(g_threadID, AETHER_MSG_MOUSE_BUTTON, wParam, data);
    return (g_mode == kHOOK_RELAY_EVENTS);

  case WM_MOUSEWHEEL:
    if (g_mode == kHOOK_RELAY_EVENTS) {
      // relay event
      PostThreadMessage(g_threadID, AETHER_MSG_MOUSE_WHEEL, data, 0);
    }
    return (g_mode == kHOOK_RELAY_EVENTS);
  ...
  case WM_MOUSEHWHEEL:
    if (g_mode == kHOOK_RELAY_EVENTS) {
      // relay event
      PostThreadMessage(g_threadID, AETHER_MSG_MOUSE_WHEEL, 0, data);
    }
    return (g_mode == kHOOK_RELAY_EVENTS);
  ...
  case WM_MOUSEMOVE:
    if (g_mode == kHOOK_RELAY_EVENTS) {
      // relay and eat event
      PostThreadMessage(g_threadID, AETHER_MSG_MOUSE_MOVE, x, y);
      return true;
  ...
  if (code >= 0) {
    if (g_screenSaver) {
      ...
      if (msg->message == WM_SYSCOMMAND && msg->wParam == SC_SCREENSAVE) {
        ...
        PostThreadMessage(g_threadID, AETHER_MSG_SCREEN_SAVER, TRUE, 0);
      }
    }
  }

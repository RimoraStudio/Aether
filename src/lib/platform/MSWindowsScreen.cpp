...
void MSWindowsScreen::checkClipboards()
{
  ...
  if (m_ownClipboard && !MSWindowsClipboard::isOwnedByAether()) {
...
void MSWindowsScreen::warpCursor(int32_t x, int32_t y)
{
  ...
  MSG msg;
  while (PeekMessage(&msg, nullptr, AETHER_MSG_INPUT_FIRST, AETHER_MSG_INPUT_LAST, PM_REMOVE)) {
    // do nothing
  }
...
bool MSWindowsScreen::onPreDispatch(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  // handle event
  switch (message) {
  case AETHER_MSG_SCREEN_SAVER:
    return onScreensaver(wParam != 0);

  case AETHER_MSG_DEBUG:
...
bool MSWindowsScreen::onPreDispatchPrimary(HWND, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case AETHER_MSG_PRE_WARP: {
    ...
    MSG msg;
    do {
      GetMessage(&msg, nullptr, AETHER_MSG_MOUSE_MOVE, AETHER_MSG_POST_WARP);
    } while (msg.message != AETHER_MSG_POST_WARP);
  }
    return true;

  case AETHER_MSG_POST_WARP:
...
//   1. aether checks the mouse position on server screen
...
bool MSWindowsScreen::onScreensaver(bool activated)
{
  ...
  MSG msg;
  if (PeekMessage(&msg, nullptr, AETHER_MSG_SCREEN_SAVER, AETHER_MSG_SCREEN_SAVER, PM_NOREMOVE)) {
    return true;
  }

  if (activated) {
    if (!m_screensaverActive && m_screensaver->checkStarted(AETHER_MSG_SCREEN_SAVER, FALSE, 0)) {
...
void MSWindowsScreen::onClipboardChange()
{
  ...
  // we're the owner).
  if (!MSWindowsClipboard::isOwnedByAether()) {
    if (m_ownClipboard) {
...
void MSWindowsScreen::warpCursorNoFlush(int32_t x, int32_t y)
{
  ...
  PostThreadMessage(GetCurrentThreadId(), AETHER_MSG_PRE_WARP, x, y);
  ...
  PostThreadMessage(GetCurrentThreadId(), AETHER_MSG_POST_WARP, 0, 0);
}
...
void MSWindowsScreen::nextMark()
{
  ...
  PostThreadMessage(GetCurrentThreadId(), AETHER_MSG_MARK, m_mark, 0);
}

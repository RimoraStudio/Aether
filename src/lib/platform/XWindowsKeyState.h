...
#else
#error The XTest extension is required to build aether
#endif
...
public:
  ...
  XWindowsKeyState(Display *, bool useXKB, IEventQueue *events, aether::KeyMap &keyMap);
  ...
  */
  ...
  //! Convert X modifier mask to aether mask
  /*!
  Returns the aether modifier mask corresponding to the X modifier
  mask in \p state.
  */
  KeyModifierMask mapModifiersFromX(unsigned int state) const;

  //! Convert aether modifier mask to X mask
  /*!
  Converts the aether modifier mask to the corresponding X modifier
  mask.  Returns \c true if successful and \c false if any modifier
  could not be converted.
  */
  bool mapModifiersToX(KeyModifierMask, unsigned int &) const;

  //! Convert aether key to all corresponding X keycodes
  /*!
  Converts the aether key \p key to all of the keycodes that map to
  that key.
  */
...
protected:
  // KeyState overrides
  void getKeyMap(aether::KeyMap &keyMap) override;
  void fakeKey(const Keystroke &keystroke) override;

private:
  void init(bool useXKB);
  void updateKeysymMap(aether::KeyMap &);
  void updateKeysymMapXKB(aether::KeyMap &);
  ...
  */
  ...
  static void remapKeyModifiers(KeyID, int32_t, aether::KeyMap::KeyItem &, void *);
...
private:
  ...
  // X modifier (bit number) to aether modifier (mask) mapping
  KeyModifierMaskList m_modifierFromX = {};

  // aether modifier (mask) to X modifier (mask)

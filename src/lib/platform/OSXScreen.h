...
#include "aether/PlatformScreen.h"
...
private:
  ...
  // map aether mouse button to mac buttons
  ButtonID mapAetherButtonToMac(uint16_t) const;

  // map mac mouse button to aether buttons
  ButtonID mapMacButtonToAether(uint16_t) const;

  // map mac scroll wheel value to an aether scroll wheel value
  int32_t mapScrollWheelToAether(int32_t) const;

...
public:
  ...
  empty(), isOwnedByAether() will return false when emptied
  this way.  This is useful when aether wants to put data on
  clipboard but pretend (to itself) that some other app did it.
  When using empty(), aether assumes the data came from the
  server and doesn't need to be sent back.  emptyUnowned()
  makes aether send the data to the server.
  */
  bool emptyUnowned();

  //! Test if clipboard is owned by aether
  static bool isOwnedByAether();

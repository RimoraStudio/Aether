...
void OSXKeyState::getKeyMap(aether::KeyMap &keyMap)
{
...
void OSXKeyState::getKeyMapForSpecialKeys(aether::KeyMap &keyMap, int32_t group) const
{
...
bool OSXKeyState::getKeyMap(aether::KeyMap &keyMap, int32_t group, const IOSXKeyResource &r) const
{
  if (!r.isValid()) {
    return false;
  }
  ...
  for (uint32_t i = 0; i < r.getNumButtons(); ++i) {
    ...
    for (uint32_t j = 0; j < r.getNumTables(); ++j) {
      ...
      buttonKeys[j].second = aether::KeyMap::isDeadKey(buttonKeys[j].first);
    }
...
bool OSXKeyState::mapAetherHotKeyToMac(

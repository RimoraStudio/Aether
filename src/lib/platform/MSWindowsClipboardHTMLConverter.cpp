...
std::string MSWindowsClipboardHTMLConverter::doFromIClipboard(const std::string &data) const
{
  ...
  prefix.replace(prefix.find("XXXXXXXXXX"), 10, aether::string::sprintf("%010u", StartFragment));
  prefix.replace(prefix.find("YYYYYYYYYY"), 10, aether::string::sprintf("%010u", EndFragment));
  prefix.replace(prefix.find("ZZZZZZZZZZ"), 10, aether::string::sprintf("%010u", EndHTML));

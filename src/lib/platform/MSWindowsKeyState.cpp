...
// map virtual keys to aether key enumeration
...
MSWindowsKeyState::MSWindowsKeyState(
    MSWindowsDesks *desks, void *eventTarget, IEventQueue *events, aether::KeyMap &keyMap,
...
void MSWindowsKeyState::getKeyMap(aether::KeyMap &keyMap)
{
...
KeyID MSWindowsKeyState::getIDForKey(
    aether::KeyMap::KeyItem &item, KeyButton button, UINT virtualKey, PBYTE keyState, HKL hkl
) const
{
  WCHAR unicode[2];
  ...
  case -1: {
    ...
    return aether::KeyMap::getDeadKey(id);
  }
  default:
  case 0:
    // unmapped
...
void MSWindowsKeyState::addKeyEntry(aether::KeyMap &keyMap, aether::KeyMap::KeyItem &item)
{

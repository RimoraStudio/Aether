...
uint32_t XWindowsScreen::registerHotKey(KeyID key, KeyModifierMask mask)
{
  ...
  if (err) {
    ...
    m_hotKeys.erase(id);
    LOG(
        (CLOG_WARN "failed to register hotkey %s (id=%04x mask=%04x)", aether::KeyMap::formatKey(key, mask).c_str(),
         key, mask)
    );
    return 0;
  }
  ...
  LOG(
      (CLOG_DEBUG "registered hotkey %s (id=%04x mask=%04x) as id=%d", aether::KeyMap::formatKey(key, mask).c_str(),
       key, mask, id)
  );
  return id;
}
...
void XWindowsScreen::setShape(int32_t width, int32_t height)
{
  ...
  // second (unexpected) warp causes aether to think the pointer
...
void XWindowsScreen::openIM()
{
  ...
  // find the appropriate style.  aether supports XIMPreeditNothing
  // only at the moment.
  XIMStyles *styles;

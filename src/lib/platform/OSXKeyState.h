...
public:
  ...
  OSXKeyState(IEventQueue *events, aether::KeyMap &keyMap, std::vector<std::string> layouts, bool isLangSyncEnabled);
  ...
  */
  ...
  //! Convert OS X modifier mask to aether mask
  /*!
  Returns the aether modifier mask corresponding to the OS X modifier
  mask in \p mask.
  */
  ...
  */
  bool
  mapAetherHotKeyToMac(KeyID key, KeyModifierMask mask, uint32_t &macVirtualKey, uint32_t &macModifierMask) const;
...
private:
  ...
  // Add hard coded special keys to an aether::KeyMap.
  void getKeyMapForSpecialKeys(aether::KeyMap &keyMap, int32_t group) const;

  // Convert keyboard resource to a key map
  bool getKeyMap(aether::KeyMap &keyMap, int32_t group, const IOSXKeyResource &r) const;
...
private:
  // OS X uses a physical key if 0 for the 'A' key.  aether reserves

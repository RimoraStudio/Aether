...
HANDLE openProcessForKill(const PROCESSENTRY32 &entry)
{
  ...
  if (_wcsicmp(entry.szExeFile, L"aether-client.exe") != 0 && //
      _wcsicmp(entry.szExeFile, L"aether-server.exe") != 0 && //
      _wcsicmp(entry.szExeFile, L"aether-core.exe") != 0) {
    return nullptr;
  }
...
void MSWindowsWatchdog::startProcess()
{
  ...
  m_process = std::make_unique<aether::platform::MSWindowsProcess>(m_command, m_outputWritePipe, m_outputWritePipe);
...
void MSWindowsWatchdog::shutdownExistingProcesses()
{
  ...
  while (gotEntry) {
    ...
    if (HANDLE handle = openProcessForKill(entry); handle != nullptr) {
      ...
      aether::platform::MSWindowsProcess::shutdown(handle, entry.th32ProcessID);
      CloseHandle(handle);
    }

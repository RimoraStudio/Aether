...
XWindowsKeyState::XWindowsKeyState(Display *display, bool useXKB, IEventQueue *events, aether::KeyMap &keyMap)
    : KeyState(
...
void XWindowsKeyState::getKeyMap(aether::KeyMap &keyMap)
{
  // get autorepeat info.  we must use the global_auto_repeat told to
  // us because it may have modified by aether.
...
void XWindowsKeyState::updateKeysymMap(aether::KeyMap &keyMap)
{
  ...
  for (int i = 0; i < numKeycodes; ++i) {
    ...
    for (int j = 0; j < maxKeysyms; ++j) {
      ...
      if (modifierButtons.contains(keycode)) {
        ...
        aether::KeyMap::initModifierKey(item);
...
#if HAVE_XKB_EXTENSION
void XWindowsKeyState::updateKeysymMapXKB(aether::KeyMap &keyMap)
{
  ...
  // change all modifier masks to aether masks from X masks
...
void XWindowsKeyState::remapKeyModifiers(KeyID, int32_t group, aether::KeyMap::KeyItem &item, void *vself)
{

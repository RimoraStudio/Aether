...
void EiScreen::fakeMouseWheel(ScrollDelta delta) const
{
  ...
  // libei and aether seem to use opposite directions, so we have
...
{
  // both libei and aether use multiples of 120 to represent
...
void EiScreen::handleSystemEvent(const Event &)
{
  ...
  while ((event = ei_get_event(m_ei)) != nullptr) {
    ...
    case EI_EVENT_DISCONNECT:
      ...
      // so that the cursor is usable and not stuck on the aether server.

 ...
 * SPDX-FileCopyrightText: (C) 2025 - 2026 Rimora Studio
...
#include "aether/IScreenSaver.h"
#include "aether/ScreenException.h"
#include "aether/win32/AppUtilWindows.h"
#include "mt/Lock.h"
#include "mt/Thread.h"
...
// <unused>; <unused>
#define AETHER_MSG_SWITCH AETHER_HOOK_LAST_MSG + 1
// <unused>; <unused>
#define AETHER_MSG_ENTER AETHER_HOOK_LAST_MSG + 2
// <unused>; <unused>
#define AETHER_MSG_LEAVE AETHER_HOOK_LAST_MSG + 3
// wParam = flags, HIBYTE(lParam) = virtual key, LOBYTE(lParam) = scan code
#define AETHER_MSG_FAKE_KEY AETHER_HOOK_LAST_MSG + 4
// flags, XBUTTON id
#define AETHER_MSG_FAKE_BUTTON AETHER_HOOK_LAST_MSG + 5
// x; y
#define AETHER_MSG_FAKE_MOVE AETHER_HOOK_LAST_MSG + 6
// xDelta; yDelta
#define AETHER_MSG_FAKE_WHEEL AETHER_HOOK_LAST_MSG + 7
// POINT*; <unused>
#define AETHER_MSG_CURSOR_POS AETHER_HOOK_LAST_MSG + 8
// IKeyState*; <unused>
#define AETHER_MSG_SYNC_KEYS AETHER_HOOK_LAST_MSG + 9
// install; <unused>
#define AETHER_MSG_SCREENSAVER AETHER_HOOK_LAST_MSG + 10
// dx; dy
#define AETHER_MSG_FAKE_REL_MOVE AETHER_HOOK_LAST_MSG + 11
// enable; <unused>
#define AETHER_MSG_FAKE_INPUT AETHER_HOOK_LAST_MSG + 12
...
{
  sendMessage(AETHER_MSG_ENTER, 0, 0);
}

void MSWindowsDesks::leave(HKL keyLayout)
{
  sendMessage(AETHER_MSG_LEAVE, (WPARAM)keyLayout, 0);
}
...
{
  sendMessage(AETHER_MSG_SYNC_KEYS, 0, 0);
}
...
void MSWindowsDesks::installScreensaverHooks(bool install)
{
  if (m_isPrimary && m_screensaverNotify != install) {
    ...
    sendMessage(AETHER_MSG_SCREENSAVER, install, 0);
  }
}

void MSWindowsDesks::fakeInputBegin()
{
  sendMessage(AETHER_MSG_FAKE_INPUT, 1, 0);
}

void MSWindowsDesks::fakeInputEnd()
{
  sendMessage(AETHER_MSG_FAKE_INPUT, 0, 0);
}

void MSWindowsDesks::getCursorPos(int32_t &x, int32_t &y) const
{
  POINT pos{0, 0};
  sendMessage(AETHER_MSG_CURSOR_POS, reinterpret_cast<WPARAM>(&pos), 0);
  x = pos.x;
  y = pos.y;
}
...
{
  sendMessage(AETHER_MSG_FAKE_KEY, flags, MAKELPARAM(scanCode, virtualKey));
}
...
void MSWindowsDesks::fakeMouseButton(ButtonID button, bool press)
{
  ...
  // do it
  sendMessage(AETHER_MSG_FAKE_BUTTON, flags, data);
}

void MSWindowsDesks::fakeMouseMove(int32_t x, int32_t y) const
{
  sendMessage(AETHER_MSG_FAKE_MOVE, static_cast<WPARAM>(x), static_cast<LPARAM>(y));
}

void MSWindowsDesks::fakeMouseRelativeMove(int32_t dx, int32_t dy) const
{
  sendMessage(AETHER_MSG_FAKE_REL_MOVE, static_cast<WPARAM>(dx), static_cast<LPARAM>(dy));
}

void MSWindowsDesks::fakeMouseWheel(int32_t xDelta, int32_t yDelta) const
{
  sendMessage(AETHER_MSG_FAKE_WHEEL, xDelta, yDelta);
}
...
ATOM MSWindowsDesks::createDeskWindowClass(bool isPrimary) const
{
  ...
  classInfo.lpszClassName = L"AetherDesk";
...
void MSWindowsDesks::deskMouseRelativeMove(int32_t dx, int32_t dy) const
{
  ...
  // system.  that defeats the purpose of aether so we'll assume
...
void MSWindowsDesks::deskLeave(Desk *desk, HKL keyLayout)
{
  ...
  if (m_isPrimary) {
    ...
    // disable the window (see handling of AETHER_MSG_SWITCH).
    if (!desk->m_lowLevel) {
...
void MSWindowsDesks::deskThread(const void *vdesk)
{
  ...
  if (desk->m_desk != nullptr && SetThreadDesktop(desk->m_desk) != 0) {
    ...
    try {
      desk->m_window = createWindow(m_deskClass, L"AetherDesk");
  ...
  while (GetMessage(&msg, nullptr, 0, 0)) {
    ...
    case AETHER_MSG_ENTER:
      m_isOnScreen = true;
      deskEnter(desk);
      break;

    case AETHER_MSG_LEAVE:
      m_isOnScreen = false;
      m_keyLayout = (HKL)msg.wParam;
      deskLeave(desk, m_keyLayout);
      break;

    case AETHER_MSG_FAKE_KEY:
    ...
    case AETHER_MSG_SYNC_KEYS:
      m_updateKeys->run();
      break;

    case AETHER_MSG_SCREENSAVER:
      if (m_useHooks) {
        if (msg.wParam != 0) {
          MSWindowsHook::installScreenSaver();
        } else {
    ...
    case AETHER_MSG_FAKE_INPUT:
      send_keyboard_input(
          AETHER_HOOK_FAKE_INPUT_VIRTUAL_KEY, AETHER_HOOK_FAKE_INPUT_SCANCODE, msg.wParam ? 0 : KEYEVENTF_KEYUP
      );
      break;
    }
...
void MSWindowsDesks::checkDesk()
{
  ...
  if (name != m_activeDeskName && !m_screensaver->isActive()) {
    ...
    sendMessage(AETHER_MSG_SWITCH, 0, 0);

    // hide cursor on new desk
    if (!wasOnScreen) {
      sendMessage(AETHER_MSG_LEAVE, (WPARAM)m_keyLayout, 0);
    }
    ...
    PostThreadMessage(m_threadID, AETHER_MSG_SCREEN_SAVER, TRUE, 0);
  }
}
...
void MSWindowsDesks::handleCheckDesk()
{
  ...
  if (m_isPrimary) {
    ...
    PostThreadMessage(m_threadID, AETHER_MSG_SCREEN_SAVER, running, 0);
  }
}

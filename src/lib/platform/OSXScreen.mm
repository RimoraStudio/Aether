...
#include "aether/ClientApp.h"
#include "aether/Clipboard.h"
#include "aether/DisplayInvalidException.h"
#include "aether/KeyMap.h"
...
enum
{
  kAetherEventMouseScroll = 11,
  kAetherMouseScrollAxisX = 'saxx',
  kAetherMouseScrollAxisY = 'saxy'
};
...
{
  // get mac virtual key and modifier mask matching aether key and mask
  uint32_t macKey, macMask;
  if (!m_keyState->mapAetherHotKeyToMac(key, mask, macKey, macMask)) {
  ...
  if (!okay) {
    ...
    LOG_WARN(
        "failed to register hotkey %s (id=%04x mask=%04x)", aether::KeyMap::formatKey(key, mask).c_str(), key, mask
    );
    return 0;
  }
  ...
  LOG_DEBUG(
      "registered hotkey %s (id=%04x mask=%04x) as id=%d", aether::KeyMap::formatKey(key, mask).c_str(), key, mask, id
  );
  return id;
}
...
void OSXScreen::fakeMouseButton(ButtonID id, bool press)
{
  ...
  uint32_t index = mapAetherButtonToMac(id) - kButtonLeft;
...
void OSXScreen::handleSystemEvent(const Event &event)
{
  ...
  case kEventClassMouse:
    ...
    case kAetherEventMouseScroll: {
      ...
      // get scroll amount
      r = GetEventParameter(
          *carbonEvent, kAetherMouseScrollAxisX, typeSInt32, nullptr, sizeof(xScroll), nullptr, &xScroll
      );
      if (r != noErr) {
        xScroll = 0;
      }
      r = GetEventParameter(
          *carbonEvent, kAetherMouseScrollAxisY, typeSInt32, nullptr, sizeof(yScroll), nullptr, &yScroll
      );
      if (r != noErr) {
        yScroll = 0;
      }
      ...
      if (xScroll != 0 || yScroll != 0) {
        onMouseWheel(-mapScrollWheelToAether(xScroll), mapScrollWheelToAether(yScroll));
      }
    }
    }
    break;
...
bool OSXScreen::onMouseButton(bool pressed, uint16_t macButton)
{
  ...
  ButtonID button = mapMacButtonToAether(macButton);
...
ButtonID OSXScreen::mapAetherButtonToMac(uint16_t button) const
{
  switch (button) {
  case 1:
    return kButtonLeft;
...
ButtonID OSXScreen::mapMacButtonToAether(uint16_t macButton) const
{
  switch (macButton) {
  case 1:
    return kButtonLeft;
...
int32_t OSXScreen::mapScrollWheelToAether(int32_t x) const
{
...
// will show the cursor if a local event comes in while aether has the cursor
// off the screen.
CGEventRef
...
CGEventRef OSXScreen::handleCGInputEvent(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon)
{
  ...
  case kCGEventScrollWheel:
    screen->onMouseWheel(
        screen->mapScrollWheelToAether(CGEventGetIntegerValueField(event, kCGScrollWheelEventDeltaAxis2)),
        screen->mapScrollWheelToAether(CGEventGetIntegerValueField(event, kCGScrollWheelEventDeltaAxis1))
    );
    break;

...
{
  return format(
      "IncompatibleClientException", "incompatible client %{1}.%{2}", aether::string::sprintf("%d", m_major).c_str(),
      aether::string::sprintf("%d", m_minor).c_str()
  );
}
...
{
  return format("ExitAppException", "exiting with code %{1}", aether::string::sprintf("%d", m_code).c_str());
}

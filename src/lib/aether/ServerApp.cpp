...
#include "aether/Screen.h"
#include "aether/ScreenException.h"
#include "aether/ipc/CoreIpc.h"
...
void ServerApp::parseArgs()
{
  if (const auto address = Settings::value(Settings::Core::Interface).toString(); !address.isEmpty()) {
    ...
    *m_aetherAddress = NetworkAddress(Settings::value(Settings::Core::Port).toInt());
  }

  try {
    m_aetherAddress->resolve();
...
void ServerApp::closeServerScreen(aether::Screen *screen)
{
  if (screen != nullptr) {
...
bool ServerApp::initServer()
{
  ...
  aether::Screen *serverScreen = nullptr;
...
aether::Screen *ServerApp::openServerScreen()
{
  aether::Screen *screen = createScreen();
...
aether::Screen *ServerApp::createScreen()
{
#if defined(Q_OS_WIN)
  return new aether::Screen(
      new MSWindowsScreen(true, Settings::value(Settings::Core::UseHooks).toBool(), getEvents()), getEvents()
  );
#elif defined(Q_OS_MACOS)
  return new aether::Screen(new OSXScreen(getEvents(), true), getEvents());
#else
  if (aether::platform::isWayland()) {
#if WINAPI_LIBEI
    LOG_INFO("using ei screen for wayland");
    return new aether::Screen(new aether::EiScreen(true, getEvents(), true), getEvents());
#else
    throw XNoEiSupport();
#endif
  }
...
PrimaryClient *ServerApp::openPrimaryClient(const std::string &name, aether::Screen *screen)
{
...
int ServerApp::mainLoop()
{
  ...
  // the default.
  if (m_aetherAddress->isValid()) {
    m_config->setAetherAddress(*m_aetherAddress);
  } else if (!m_config->getAetherAddress().isValid()) {
    m_config->setAetherAddress(NetworkAddress(kDefaultPort));
  }
...
int ServerApp::runInner(StartupFunc startup)
{
  ...
  m_aetherAddress = new NetworkAddress;
  m_config = std::make_shared<Config>(getEvents());

  // run
  int result = startup();

  delete m_aetherAddress;
  return result;
}
...
{
  if (aether::platform::isWindows())
    return "Aether Server";
  return "aether-server";
}

...
public:
  ...
  KeyState(IEventQueue *events, aether::KeyMap &keyMap, std::vector<std::string> layouts, bool isLangSyncEnabled);
  ~KeyState() override;
  ...
  */
  ...
  void updateKeyMap(aether::KeyMap *existing);
  // IKeyState overrides
...
protected:
  using Keystroke = aether::KeyMap::Keystroke;
  ...
  */
  virtual void getKeyMap(aether::KeyMap &keyMap) = 0;
...
private:
  using Keystrokes = aether::KeyMap::Keystrokes;
  using ModifierToKeys = aether::KeyMap::ModifierToKeys;
...
private:
  ...
  public:
    bool operator()(
        const aether::KeyMap::ButtonToKeyMap::value_type &a, const aether::KeyMap::ButtonToKeyMap::value_type b
    ) const
    {
  ...
  static void addActiveModifierCB(KeyID id, int32_t group, aether::KeyMap::KeyItem &keyItem, void *vcontext);

private:
  // must be declared before m_keyMap. used when this class owns the key map.
  aether::KeyMap *m_keyMapPtr;

  // the keyboard map
  aether::KeyMap &m_keyMap;

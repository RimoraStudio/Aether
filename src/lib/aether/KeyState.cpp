...
KeyState::KeyState(
    IEventQueue *events, aether::KeyMap &keyMap, std::vector<std::string> layouts, bool isLangSyncEnabled
)
    : IKeyState(events),
      m_keyMapPtr(nullptr),
      m_keyMap(keyMap),
...
void KeyState::updateKeyMap(aether::KeyMap *existing)
{
  if (existing) {
    m_keyMap.swap(*existing);
  } else {
    // get the current keyboard map
    aether::KeyMap keyMap;
    getKeyMap(keyMap);
...
void KeyState::addActiveModifierCB(KeyID, int32_t group, aether::KeyMap::KeyItem &keyItem, void *vcontext)
{
...
void KeyState::fakeKeyDown(KeyID id, KeyModifierMask mask, KeyButton serverID, const std::string &lang)
{
  ...
  const aether::KeyMap::KeyItem *keyItem =
...
{
  const aether::KeyMap::KeyItemList *items = m_keyMap.findCompatibleKey(id, group, 0, 0);
  if (items == nullptr) {
    return 0;
  } else {
...
)
{
  ...
  aether::KeyMap::ButtonToKeyMap oldKeys;
  aether::KeyMap::ButtonToKeyMap newKeys;
  ...
  aether::KeyMap::ButtonToKeyMap pressed;
  aether::KeyMap::ButtonToKeyMap released;
  ...
  // update state
  for (aether::KeyMap::ButtonToKeyMap::const_iterator i = released.begin(); i != released.end(); ++i) {
    if (i->first != button) {
      m_keys[i->first] = 0;
      m_syntheticKeys[i->first] = 0;
    }
  }
  for (aether::KeyMap::ButtonToKeyMap::const_iterator i = pressed.begin(); i != pressed.end(); ++i) {
    if (i->first != button) {
      m_keys[i->first] = 1;

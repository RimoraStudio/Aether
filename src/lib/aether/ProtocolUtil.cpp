...
#include "base/Log.h"
#include "aether/AetherException.h"
...
void ProtocolUtil::writef(aether::IStream *stream, const char *fmt, ...)
{
...
bool ProtocolUtil::readf(aether::IStream *stream, const char *fmt, ...)
{
  bool result = false;
...
void ProtocolUtil::vwritef(aether::IStream *stream, const char *fmt, uint32_t size, va_list args)
{
  assert(stream != nullptr);
  assert(fmt != nullptr);
...
void ProtocolUtil::vreadf(aether::IStream *stream, const char *fmt, va_list args)
{
  assert(stream != nullptr);
  assert(fmt != nullptr);
...
void ProtocolUtil::read(aether::IStream *stream, void *vbuffer, uint32_t count)
{
  assert(stream != nullptr);
...
uint8_t ProtocolUtil::read1ByteInt(aether::IStream *stream)
{
...
uint16_t ProtocolUtil::read2BytesInt(aether::IStream *stream)
{
...
uint32_t ProtocolUtil::read4BytesInt(aether::IStream *stream)
{
...
void ProtocolUtil::readVector1ByteInt(aether::IStream *stream, std::vector<uint8_t> &destination)
{
...
void ProtocolUtil::readVector2BytesInt(aether::IStream *stream, std::vector<uint16_t> &destination)
{
  auto size = readVectorSize(stream);
  for (auto i = 0; i < size; ++i) {
...
void ProtocolUtil::readVector4BytesInt(aether::IStream *stream, std::vector<uint32_t> &destination)
{
  auto size = readVectorSize(stream);
  for (auto i = 0; i < size; ++i) {
...
uint32_t ProtocolUtil::readVectorSize(aether::IStream *stream)
{
...
void ProtocolUtil::readBytes(aether::IStream *stream, uint32_t len, std::string *destination)
{

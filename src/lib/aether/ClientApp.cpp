...
aether::Screen *ClientApp::createScreen()
{
#if defined(Q_OS_WIN)
  ...
  if (aether::platform::isWayland()) {
    ...
    return new aether::Screen(new aether::EiScreen(false, getEvents(), true), getEvents());
#else
    throw XNoEiSupport();
#endif
  }
...
aether::Screen *ClientApp::openClientScreen()
{
...
void ClientApp::closeClientScreen(aether::Screen *screen)
{
  if (screen != nullptr) {
...
void ClientApp::handleClientConnected()
{
  ...
  ipcSendConnectionState(aether::core::ConnectionState::Connected);
...
Client *ClientApp::openClient(const std::string &name, const NetworkAddress &address, aether::Screen *screen)
{
...
{
  aether::Screen *clientScreen = nullptr;
  try {

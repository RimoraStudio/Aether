...
To build Aether you will a minimum of: 
    - [cmake] 3.24+
    - [Qt] 6.7.0+
    - [openssl] 3.0+
...
By default a build of Aether will: 
     - The GUI application `aether`
     - The Core application `aether-core`
     - Documentation if [doxygen] was found on your system
     - Tests that will be run as part of the build process

## Configuration

Aether supports the following CMake options:
...
### Windows Configuration
 ...
 It is recommended to use vcpkg to install the dependencies. The first time you configure Aether, all dependencies other than Qt will be built. If you don't want to use vcpkg, you must manually setup the dependencies. However, that will not be covered by this document.
...
8. To verify, run: `codesign -d -r- build/bin/Aether.app`
...
## Making Aether packages

 Aether can generate several packages using `cpack`.
 ...
 Aether can generate several package types depending on the system. 

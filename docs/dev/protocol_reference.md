...
This document provides a comprehensive reference for the Aether network protocol. It is the primary source of information for developers implementing Aether clients or extending the protocol.

## Protocol Overview

The Aether protocol enables keyboard and mouse sharing between multiple computers over a TCP network connection. The protocol uses two distinct sets of terminology to describe the roles of the computers involved:
...
Client                                 Server
  ...
  |                                      |
  | ◄─────────────────────────────────── |
  | "Aether" + version (1.8)           | Hello message
  |                                      |
  | "Aether" + version + name          |
  | ───────────────────────────────────► | HelloBack message
  |                                      |
...
- **Protocol Logs**: Enable verbose logging in Aether

# GUI Config
 ...
 Aether will automatically figure out where to save settings and other files.

## Search paths

Aether will look for settings in several places depending on your operating system.
The search order for a setting file depends on your operating system

### Linux

  1. `<XDG_CONFIG_HOME>/Aether/Aether.conf`
  2. `~/.config/Aether/Aether.conf`
  3. `/etc/Aether/Aether.conf`
...
### macOS
  ...
  1. `~/Library/Aether/Aether.conf`
  2. `/Library/Aether/Aether.conf`
...
### Windows
  ...
  1. `<install-path>/settings/Aether.conf`
  2. Windows Registry `HKCU\Software\Aether\Aether`

Windows will save to the install dir if settings are loaded from there. If not, it saves any other config files in: `C:\ProgramData\Aether\`
...
| preventSleep  | `true` or `false` | Prevent sleep when Aether is active [default: false] |
...
|Option | Valid Values|Description|
|:----------|:-----------:|:-----------|
| command   | Filename          | The filename of the binary the daemon. This binary exists in the same path as the aether GUI |
...
| updateCheckUrl                 | URL               | The URL to use when checking for a new version number, it should return a version [default: https://api.github.com/repos/RimoraStudio/Aether/releases/latest]|
...
|Option              |    Valid Values   |Description|
|:-------------------|:-----------------:|:-----------|
| clipboardSize      | int > 0           | Aether will send a maximum of `N` megabytes of clipboard data to another computer when the mouse transitions to that computer.|
...
| heartbeat          | int               | The server will expect each client to send a message no less than every `N` milliseconds. If no message arrives from a client within `3N` seconds the server forces that client to disconnect. If aether fails to detect clients disconnecting while the server is sleeping or vice versa, try using this option.|
...
|relativeMouseMoves  | `true` or `false` | If set to ''true'' then secondary computers move the mouse using relative rather than absolute mouse moves when and only when the cursor is locked to the computer (by ''Scroll Lock'' or a configured hot key). This is intended to make Aether work better with certain games. If set to ''false'' or not set then all mouse moves are absolute.|
| switchDelay        | int               | Aether won't switch computers when the mouse reaches edge of a computer unless it stays on the edge for `N` milliseconds. This helps prevent unintentional switching when working near an edge. (default: 250)|
| switchDoubleTap    | int               | Aether won't switch computers when the mouse reaches the edge of a computer unless it's moved away from the edge and then back to the edge within `N` milliseconds. With the option you have to quickly tap the edge twice to switch. This helps prevent unintentional switching when working near the edge.|
|win32KeepForeground | `true` or `false` | If set to ''true'' (the default), Aether will grab the foreground focus on a Windows server (thereby putting all other windows in the background) upon switching to a client. If set to ''false'', it will leave the currently foreground window in the foreground. Aether grabs the focus to avoid issues with other apps interfering with Aether's ability to read the hardware inputs. |

 - You can use both the ''switchDelay'' and ''switchDoubleTap'' options at the same time. Aether will switch when either requirement is satisfied.
...
The `aether-server` command accepts the `-c` or `--config` option, which takes one argument,
...
|:--------|:-----------:|:-----------|
|screenSaverSync| `true` or `false`| ''Note: Removed in v1.14.1'' If set to ''false'' then Aether won't synchronize screen savers. Client screen savers will start according to their individual configurations. The server screen saver won't start if there is input, even if that input is directed toward a client computer.|
...
This example comes from doc/aether-basic.conf

```
# sample aether configuration file
#
...
```
# Physical monitor arrangement, with machine names as used by Aether.
#  +----------+----------+
#  | syn-serv | syn-cli  |
#  |          |          |
#  +----------+----------+ 
...
See also: the man page for ''aether-core''.
